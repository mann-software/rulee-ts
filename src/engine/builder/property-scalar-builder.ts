...
import { RulesDefinition } from "../../rules/rules-definition";
...
export class PropertyScalarBuilder {
    ...
    simpleProperty<T>(id: PropertyId, valueConverter: ValueConverter<T>, config?: PropertyScalarValueConfig<T>, ...rulesdefinitions: RulesDefinition<T>[]): PropertyScalar<T> {
    ...
    stringProperty(id: PropertyId, config?: PropertyScalarValueConfig<string>, ...rulesdefinitions: RulesDefinition<string>[]): PropertyScalar<string> {
    ...
    numberProperty(id: PropertyId, config?: PropertyScalarValueConfig<number> & { zeroIsConsideredAsEmpty?: boolean }, ...rulesdefinitions: RulesDefinition<number>[]): PropertyScalar<number> {
    ...
    booleanProperty(id: PropertyId, config?: PropertyScalarValueConfig<boolean>, ...rulesdefinitions: RulesDefinition<boolean>[]): PropertyScalar<boolean> {
    ...
    dateProperty(id: PropertyId, config?: PropertyScalarValueConfig<Date>, ...rulesdefinitions: RulesDefinition<Date>[]): PropertyScalar<Date> {
    ...
    select = {
        static: <T>(id: PropertyId, choices: Choice<T>[], config?: PropertyScalarValueConfig<T> & { emptyChoice?: Choice<T> }, ...rulesdefinitions: RulesDefinition<T>[]) => {
        ...
        }, ...rulesdefinitions: RulesDefinition<T>[]) => {
    ...
    typeahead = {
        ...
        }, ...rulesdefinitions: RulesDefinition<[T | null, string]>[]): PropertyScalarWithChoices<[T | null, string], T> => {
        ...
        withPropertySource: <T, S extends PropertyArrayListReadonly<Choice<T>>>(id: PropertyId, inputSource: PropertyScalar<string>, choicesSource: S, config?: PropertyScalarConfig, ...rulesdefinitions: RulesDefinition<[T | null, string]>[]): PropertyScalarWithChoices<[T | null, string], T> => {
    ...
    objectProperty<T, O>(id: PropertyId, valueConverter: ValueConverter<T>, obj: O, config: PropertyScalarValueConfig<T> & {
        ...
        emptyValueFcn?: EmptyValueFcn<T>;
    }, ...rulesdefinitions: RulesDefinition<T>[]): PropertyScalar<T> {
    ...
    bind<T>(prop: PropertyScalar<T>, ...rulesDefintion: RulesDefinition<T>[]): void {
        const ruleBuilder = this.bindPropertyScalar(prop);
        rulesDefintion.forEach(def => def.apply(ruleBuilder));
    }

    private bindRulesAndApplyConfig<T>(prop: PropertyScalarImpl<T>, config?: PropertyScalarValueConfig<T>, ...rulesDefintion: RulesDefinition<T>[]): PropertyScalar<T> {
